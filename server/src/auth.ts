...
import { dbGetUser, dbGetUserByField, dbSaveUser, dbUpdateUser, dbUserExists } from './storage';
/**
 * Created by Ron on 02/10/2016.
 */
export const authRoutes = express
  .Router()
  .post('/login', handleAsync(login))
  .post('/signup', handleAsync(signup))
  .get('/refresh', handleAsync(refresh))
  .use(
    jwtMiddleware({
  ...
  .post('/google', handleAsync(google))
  .post('/facebook', handleAsync(facebook))
  .post('/twitter', handleAsync(twitter));

function handleAsync(func: (request: Request, response: Response) => Promise<any>) {
...
export async function refresh(request: Request, response: Response) {
  try {
    ...
    const token: ITokenUser & { exp: number } = await verifyTokenAsync(encodedToken, config.auth.TOKEN_SECRET, { ignoreExpiration: true });
    if (+token.exp * 1000 + 10 * 60 * 1000 < Date.now()) {
      //10 minutes
...
export async function login(request: Request, response: Response) {
  try {
    ...
    if (!user || !user.hash) {
      //user does not have a password, only google account
...
export async function twitter(req: RequestWithUser, res: Response) {
  ...
  if (!req.body.oauthData) {
    ...
    const { clientId } = req.body.oauthOptions;
    const { oauth_token, oauth_verifier } = req.body.oauthData;
    ...
    console.log('got token #2');
    console.dir(access_token);
    const profileOauth = {
      consumer_key: clientId,
      consumer_secret: config.auth.TWITTER_SECRET,
      token: access_token.oauth_token as string,
      token_secret: access_token.oauth_token_secret as string
    };
